...
Sub ExportModulesAsBAS()
    ...
    For Each VBComp In ActiveWorkbook.VBProject.VBComponents
        ...
        If VBComp.Type = vbext_ct_StdModule Then
            ...
            ' Read the code from the module
            ModCode = VBComp.CodeModule.Lines(1, VBComp.CodeModule.CountOfLines)
            ' Remove the  statement from the code
            ModCode = Replace(ModCode, "", "")
            ' Write the modified code to a text file
            i = FreeFile
            Open ExportPath & FileName For Output As i
            Print #i, ModCode
            Close i
        End If
    Next VBComp
...
Sub ImportBASFilesAsModules()
    ...
    Do While FileName <> ""
        ...
        ' Read the code from the BAS file and remove the  statement
